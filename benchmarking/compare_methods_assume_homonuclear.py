...
from fastoverlap import BranchnBoundAlignment    # FASTOVERLAP - Branch and Bound
...
calculate_RMSD_instead = False #True
...
#       raise ValueError("Error! The 'cell' parameter inputted must be a string of 9 numbers... Exiting!")
    ...
    # Assume molecules are homonuclear
    z = ["H" for x in z]
    ...
    if True:
#     scale = 0.6 # Set this to be ~ half interatomic separation
#     maxl = 15 # Max degree of spherical harmonic
#     FOalign = SphericalAlign(scale, maxl)

      FOalign = SphericalAlign(0.5, 15)
      FObnb = BranchnBoundAlignment()
    ...
    # Remove the original ordering of atoms in the brute-force search
    perms = perms[1:]
    ...
    # Print the header
    if True:
      print((" {:8s}# {:>6s} {:>6s}   "+" {:>8s}"*8 + "  {:>8s} ({:s})").format("DMDbound", "i", "j", "ArbAlign", "OTMol", "MolAlign", "FOa", "FOaBnB", "Umeyama", "FAQ", "GOAT", "minimum", "N"),flush=True)
    ...
    while True:
      ...
      for j,r_j in enumerate(train_R): 
        ...
        if True:
          ...
          # Method 4: FASTOVERLAP align (FOa)
          if True:
            foa_result = FOalign(r_i,r_j)
#           qapDMD = foa_result[0]*n_atoms**-0.5
            r_j_foa = foa_result[2]

            if calculate_RMSD_instead:
              RMSD1 = QQrmsd(*QQkabsch(r_i, r_j_foa))
              RMSD2 = QQrmsd(*QQkabsch(r_i, -r_j_foa))
              qapDMD = min(RMSD1,RMSD2)
            else:
              foa_DMjj = getDM(r_j_foa,lat_and_inv=lat_and_inv,invert_distances=(not calculate_RMSD_instead))
              qapDMD = np.sum((DMii - foa_DMjj)**2)

            qapDMDs["FOa"] = qapDMD

          # Method 5: FASTOVERLAP align w/ Branch and Bound (FOaBnB)
          if True:
            foa_result = FObnb(r_i,r_j)
#           qapDMD = foa_result[0]*n_atoms**-0.5
            r_j_foa = foa_result[2]
            ...
            qapDMDs["FOaBnB"] = qapDMD


          #########################################################################################################################################

          # Method 6: Do a LAP search for each atomic block SEPARATELY with a sorted distance matrix
          if True:
#           perm_qap = flapsolve(DMii,DMjj,z,z_unique)
          ...
          # Method 7: Do a QAP search with seeded FAQs for each atomic block
          if True:
          ...
          # Method 8: Do a spectral assignment (Umeyama-like)
          if True:
          ...
          # Method 9: Do a single (forward) seeded GOAT for each atomic block
          if True:
            if calculate_RMSD_instead:
          ...
          print((" {:8.4f}# {:6d} {:6d}   "+" {:8.4f}"*8 + "  {:8.4f} ({:d})").format(DMD_lowerbound, Ntraversed, j, qapDMDs["Arb"], qapDMDs["OTM"], qapDMDs["MAL"], qapDMDs["FOa"], qapDMDs["FOaBnB"], qapDMDs["spectral"], qapDMDs["FAQ"], qapDMDs["GOATf"], minDMD, NiterationsGOAT),flush=True)
